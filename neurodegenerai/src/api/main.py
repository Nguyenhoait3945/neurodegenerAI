...
from shared.lib.logging import setup_logging, get_logger, log_api_request
from shared.lib.config import get_settings, ensure_directories
from shared.lib.metrics import get_metrics_collector, record_performance
from .schemas import (
    HealthResponse, TabularPredictionRequest, MRIPredictionRequest,
    PredictionResponse, ModelMetricsResponse, FeatureImportanceResponse,
    BatchPredictionRequest, BatchPredictionResponse, ErrorResponse,
    ModelInfoResponse, CalibrationResponse, ExplanationRequest, ExplanationResponse
)
from ..models.predict import ModelPredictor, ModelEnsemble
...
ensemble_predictor: Optional[ModelEnsemble] = None
interpretability: Optional[ModelInterpretability] = None
...
async def lifespan(app: FastAPI):
    ...
    global predictor, ensemble_predictor, interpretability
...
async def predict_tabular(request: TabularPredictionRequest):
    ...
    try:
        ...
        # Log API request
        duration = (datetime.now() - start_time).total_seconds()
        log_api_request("/predict/tabular", "POST", 200, duration, request_id=request_id)
        ...
        duration = (datetime.now() - start_time).total_seconds()
        log_api_request("/predict/tabular", "POST", 500, duration, request_id=request_id)
...
async def predict_ensemble(
    tabular_request: TabularPredictionRequest,
    mri_request: MRIPredictionRequest
):
    ...
    try:
        ...
        with record_performance("ensemble_prediction"):
            prediction_result = ensemble_predictor.predict_ensemble(tabular_data, mri_volume)
        ...
        # Log API request
        duration = (datetime.now() - start_time).total_seconds()
        log_api_request("/predict/ensemble", "POST", 200, duration, request_id=request_id)
        ...
        duration = (datetime.now() - start_time).total_seconds()
        log_api_request("/predict/ensemble", "POST", 500, duration, request_id=request_id)
    ...
    try:
        ...
        with record_performance("batch_prediction", {"batch_size": len(df)}):
            if request.model_type == "tabular":
                ...
                raise HTTPException(status_code=400, detail="Only tabular batch predictions supported")
...
async def explain_prediction(request: ExplanationRequest):
    ...
    try:
        if interpretability is None:
            raise HTTPException(status_code=503, detail="Interpretability module not loaded")
        ...
        if request.model_type == "tabular":
            ...
            raise HTTPException(status_code=400, detail="Only tabular explanations supported")
