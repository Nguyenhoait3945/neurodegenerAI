...
class PostData(BaseModel):
    ...
    score: Optional[int] = Field(0, description="Post score/upvotes")
...
class ClusterData(BaseModel):
    ...
    example_posts: Optional[List[str]] = Field(None, description="Example posts from cluster")
...
class SearchRequest(BaseModel):
    ...
    time_window: Optional[str] = Field(None, description="Time window (e.g., '24h', '7d')")
...
class StreamRequest(BaseModel):
    ...
    duration_minutes: int = Field(60, ge=1, le=1440, description="Stream duration in minutes")
    posts_per_minute: int = Field(10, ge=1, le=100, description="Target posts per minute")
...
class ClusteringRequest(BaseModel):
    """Clustering request."""
    texts: List[str] = Field(..., min_items=10, max_items=10000, description="Texts to cluster")
...
class TopicAnalysisResponse(BaseModel):
    ...
    topic_words: List[Tuple[str, float]] = Field(..., description="Topic words with weights")
...
class BatchProcessingRequest(BaseModel):
    ...
    posts: List[PostData] = Field(..., min_items=1, max_items=1000, description="Posts to process")
...
class UMAPVisualizationRequest(BaseModel):
    ...
    texts: Optional[List[str]] = Field(None, description="Texts to visualize")
    embeddings: Optional[List[List[float]]] = Field(None, description="Pre-computed embeddings")
    topics: Optional[List[int]] = Field(None, description="Topic assignments")
...
class RealTimeUpdateRequest(BaseModel):
    ...
    max_posts: int = Field(1000, ge=10, le=10000, description="Maximum posts per update")
