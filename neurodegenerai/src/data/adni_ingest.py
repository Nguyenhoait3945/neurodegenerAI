...
class ADNIDataLoader(LoggerMixin):
    ...
    def load_tabular_data(self) -> Optional[pd.DataFrame]:
        ...
        self.logger.info(f"Loaded real ADNI tabular data: {len(df)} rows, {len(df.columns)} columns")
        
        return df
    
    def load_mri_data(self) -> Optional[List[np.ndarray]]:
        """Load MRI data."""
    ...
    def _load_synthetic_tabular(self) -> pd.DataFrame:
        ...
        self.logger.info(f"Diagnosis distribution: {df['DIAGNOSIS'].value_counts().to_dict()}")
    ...
    def _load_synthetic_mri(self) -> List[np.ndarray]:
        ...
        for i in range(n_samples):
            ...
            for z in range(shape[2]):
                ...
                hippo_mask = ((x - center[1])**2 + (y - center[0])**2) < (shape[0]//6)**2
                volume[hippo_mask, z] += np.random.normal(1.5, 0.3)
                
                # Cortical regions
                cortex_mask = ((x - center[1])**2 + (y - center[0])**2) < (shape[0]//3)**2
        ...
        self.logger.info(f"Generated {len(samples)} synthetic MRI samples with shape {shape}")
