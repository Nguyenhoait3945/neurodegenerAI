    ...
    def generate_gradcam(
        ...
        if save_path is not None:
            self._save_gradcam_visualization(
                input_tensor[0, 0].detach().cpu().numpy(),
                cam_resized,
                save_path
            )
...
class SHAPExplainer(LoggerMixin):
    ...
    def explain_prediction(self, instance: np.ndarray) -> Dict[str, Any]:
        ...
        feature_names = getattr(self.model, 'feature_name_', [f'feature_{i}' for i in range(len(instance))])
        ...
        explanation = {
            ...
            'prediction_proba': self.model.predict_proba(instance.reshape(1, -1))[0].tolist()
        }
    ...
    def get_feature_importance_graph(self, instances: np.ndarray, save_path: Optional[str] = None) -> None:
...
class ModelInterpretability:
    ...
    def generate_explanation_report(
        ...
        if self.model_type == "tabular":
            ...
            self.shap_explainer.get_feature_importance_graph(data_array, str(summary_plot_path))
            
        elif self.model_type == "cnn":
            # Generate Grad-CAM visualizations
            for i, (input_tensor, prediction) in enumerate(zip(data, predictions)):
                gradcam_path = save_path / f"gradcam_sample_{i}_{timestamp}.png"
