...
class TestTopicClusterer:
    ...
    def test_fit_transform(self, mock_bertopic):
        ...
        mock_model = Mock()
        mock_model.fit_transform.return_value = (np.array([0, 1, 0, 2]), np.array([[0.8, 0.2], [0.1, 0.9], [0.7, 0.3], [0.2, 0.8]]))
        mock_model.get_topic_info.return_value = Mock(to_dict=lambda x: [{'Topic': 0}, {'Topic': 1}, {'Topic': 2}])
        mock_bertopic.return_value = mock_model
...
class TestTrendAnalyzer:
    ...
    def test_get_trending_topics(self):
        ...
        topics_data = {
            ...
            2: [
                {'text': 'Topic 2 post 1', 'timestamp': '2024-01-01T10:00:00Z'}
            ]
        }
        
        embeddings_data = {
            1: np.random.randn(2, 100),
...
def test_clustering_pipeline(sample_documents):
    ...
    with patch('trend_detector.src.pipeline.cluster.BERTopic') as mock_bertopic:
        ...
        mock_model.get_topic_info.return_value = Mock(to_dict=lambda x: [
