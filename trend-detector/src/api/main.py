...
from .schemas import (
    HealthResponse, PostData, ClusterData, TrendingTopic, SearchRequest,
    SearchResponse, ClustersResponse, TrendingTopicsResponse, StreamRequest,
    StreamResponse, EmbeddingRequest, EmbeddingResponse, ClusteringRequest,
    ClusteringResponse, TopicAnalysisRequest, TopicAnalysisResponse,
    BatchProcessingRequest, BatchProcessingResponse, MetricsResponse,
    ErrorResponse, UMAPVisualizationRequest, UMAPVisualizationResponse,
    RealTimeUpdateRequest, RealTimeUpdateResponse, ModelInfoResponse
)
...
from ..pipeline.cluster import TopicClusterer, ClusterAnalyzer
...
mock_stream: Optional[MockStream] = None
reddit_stream: Optional[RedditStream] = None
...
io_utils: Optional[IOUtils] = None
                ...
                if db_handler:
                    for i, post in enumerate(posts):
                        ...
                        if post_id and embedding_generator:
                            ...
                            db_handler.insert_embedding(post_id, embedding, embedding_generator.model_name)
        ...
        if db_handler:
            ...
            for cluster_data in clusters_data:
                cluster = ClusterData(
                    ...
                    topic=cluster_data["topic"] or f"Topic {cluster_data['cluster_id']}",
                    representative_terms=cluster_data["representative_terms"].split(",") if cluster_data["representative_terms"] else [],
...
async def generate_embeddings(request: EmbeddingRequest):
    ...
    try:
        ...
        with record_performance("embedding_generation", {"num_texts": len(request.texts)}):
...
async def run_stream_task(stream_id: str, sources: List[str], duration_minutes: int, posts_per_minute: int):
        ...
        if mock_stream:
            ...
            for post in mock_stream.stream_posts(duration_minutes, posts_per_minute):
                ...
                if posts_collected % 10 == 0:
                    logger.info(f"Stream {stream_id}: collected {posts_collected} posts")
            
            logger.info(f"Stream {stream_id} completed: {posts_collected} posts collected")
