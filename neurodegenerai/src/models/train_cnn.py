...
class CNNModelTrainer(LoggerMixin):
    ...
    def train(self, epochs: int = 50, batch_size: int = 16, learning_rate: float = 0.001) -> Dict[str, Any]:
        """Train CNN model."""
    ...
    def _prepare_dataset(self, volumes: List[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _split_data(self, X: np.ndarray, y: np.ndarray, test_size: float = 0.2, val_size: float = 0.1) -> Tuple:
    ...
    def _train_epochs(self, train_loader: DataLoader, val_loader: DataLoader, epochs: int) -> None:
    ...
    def _generate_reports(self, test_loader: DataLoader, metrics: Dict[str, Any]) -> None:
    ...
    def _generate_training_plots(self, timestamp: str) -> None:
        ...
        plt.tight_layout()
        plt.savefig(plots_dir / f"training_curves_{timestamp}.png", dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    parser.add_argument("--model", choices=["simple_cnn", "resnet18"], default="simple_cnn", help="Model type")
