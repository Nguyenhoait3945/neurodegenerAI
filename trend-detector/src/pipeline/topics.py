...
class TrendAnalyzer(LoggerMixin):
    ...
    def calculate_trend_score(self, 
        ...
        try:
            ...
            volume_score = self._calculate_volume_score(current_posts, time_window_hours)
            velocity_score = self._calculate_velocity_score(topic_id, current_posts, time_window_hours)
            novelty_score = self._calculate_novelty_score(topic_id, embeddings)
            burstiness_score = self._calculate_burstiness_score(topic_id, current_posts, time_window_hours)
            ...
            # Update history
            self._update_topic_history(topic_id, {
            ...
            self.logger.error(f"Error calculating trend score for topic {topic_id}: {e}")
            return {
                'trend_score': 0.0,
                'volume_score': 0.0,
                'velocity_score': 0.0,
    ...
    def _calculate_volume_score(self, posts: List[Dict[str, Any]], time_window_hours: int) -> float:
    ...
    def _calculate_velocity_score(self, topic_id: int, posts: List[Dict[str, Any]], time_window_hours: int) -> float:
    ...
    def _calculate_burstiness_score(self, topic_id: int, posts: List[Dict[str, Any]], time_window_hours: int) -> float:
        ...
        try:
            ...
            for i in range(1, len(post_times)):
                delta = (post_times[i] - post_times[i-1]).total_seconds() / 3600  # Convert to hours
    ...
    def get_trending_topics(self, 
        ...
        try:
            ...
            for topic_id, posts in topics_data.items():
                ...
                trend_info = self.calculate_trend_score(topic_id, posts, embeddings, time_window_hours)
                ...
                trending_info = {
                    ...
                    'keywords': keywords,
                    'representative_posts': [post.get('text', '')[:100] for post in posts[:3]],
    ...
    def analyze_topic_evolution(self, topic_id: int, time_window_hours: int = 168) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate growth rate
            growth_rate = (last_score - first_score) / first_score if first_score > 0 else 0.0
