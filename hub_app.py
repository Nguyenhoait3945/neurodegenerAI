...
# Page configuration
st.set_page_config(
    page_title="Neuro-Trends Suite Hub", layout="wide", initial_sidebar_state="expanded"
)
...
def check_service_health(url: str, service_name: str) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            return {"status": "healthy", "data": response.json(), "url": url}
        else:
            return {
                "status": "unhealthy",
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">🚀 Neuro-Trends Suite Hub</h1>', unsafe_allow_html=True
    )
    ...
    with st.sidebar:
        st.image(
            "https://via.placeholder.com/200x100/1f77b4/ffffff?text=Neuro-Trends",
            width=200,
        )
    ...
    with col1:
        ...
        if neuro_health["status"] == "healthy":
            ...
            st.markdown(
                "**Status**: <span class='status-healthy'>Healthy</span>",
                unsafe_allow_html=True,
            )
            ...
            if "data" in neuro_health:
                ...
                st.markdown(
                    f"**Model Loaded**: {'✅' if health_data.get('model_loaded', False) else '❌'}"
                )
            ...
            if st.button("Test Prediction", key="neuro_test"):
                try:
                    ...
                    response = requests.post(
                        f"{NEURO_API_URL}/predict/tabular", json=test_data, timeout=10
                    )
                    if response.status_code == 200:
                        result = response.json()
                        st.success(
                            f"Test prediction successful! Result: {result.get('prediction', 'Unknown')}"
                        )
                    else:
            ...
            st.markdown("### NeuroDegenerAI API")
            st.markdown(
                "**Status**: <span class='status-unhealthy'>Unhealthy</span>",
                unsafe_allow_html=True,
            )
            ...
            if st.button("Test Search", key="trends_test"):
                try:
                    response = requests.get(
                        f"{TRENDS_API_URL}/topics/top?window=24&k=5", timeout=10
                    )
                    if response.status_code == 200:
                        result = response.json()
                        st.success(
                            f"Test search successful! Found {len(result.get('topics', []))} topics"
                        )
                    else:
