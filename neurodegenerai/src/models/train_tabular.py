...
from shared.lib.metrics import record_model_metrics, PerformanceTimer
...
class TabularModelTrainer(LoggerMixin):
    ...
    def train(self, test_size: float = 0.2, val_size: Optional[float] = None) -> Dict[str, Any]:
        """Train tabular model."""
        ...
        with PerformanceTimer("tabular_training"):
            ...
            self.model = self._train_model(X_train_processed, y_train_processed, X_val_processed, y_val)
            
            # Calibrate model
            self.logger.info("Calibrating model")
            self.calibrated_model = self._calibrate_model(X_train_processed, y_train_processed)
    ...
    def _train_model(self, X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series) -> Any:
    ...
    def _train_lightgbm(self, X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series) -> lgb.LGBMClassifier:
    ...
    def _train_xgboost(self, X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series) -> xgb.XGBClassifier:
        """Train XGBoost model."""
    ...
    def _generate_reports(self, X_test: pd.DataFrame, y_test: pd.Series, metrics: Dict[str, Any]) -> None:
    ...
    def _generate_plots(self, X_test: pd.DataFrame, y_test: pd.Series, timestamp: str) -> None:
...
def main():
    ...
    parser.add_argument("--model", choices=["lightgbm", "xgboost"], default="lightgbm", help="Model type")
